...
from datasets import prostate_dataset
...
def prostate_experiment_2():
    ...
    training_dataset = prostate_dataset.ProstateDataset(dataset_path, training_csv_path, iteration_size=-1,
                                                torchio_transforms=torchio_transforms,
                                                loading_params=loading_params,
                                                initial_thresholds=initial_thresholds,
                                                inference_transforms=inference_transforms)
    validation_dataset = prostate_dataset.ProstateDataset(dataset_path, validation_csv_path, iteration_size=-1,
                                                loading_params=loading_params,
                                                initial_thresholds=initial_thresholds,
                                                inference_transforms=inference_transforms)
    ...
    model = milnet.MILNet(**config)
    model.conv_layers.requires_grad_ = False
    
    ### Parameters ###
    experiment_name = "Chaimeleon_Prostate_MIL_EffNetV2S_2"
    learning_rate = 0.001
    save_step = 50
